...
- [新功能] Web 左侧导航新增“自选”入口，提供自选列表页支持添加/移除并一键发起分析。
- [修复] 问股 SSE 流式接口在长耗时阶段改为周期性发送进度占位事件并补充 Agent/LLM 调用耗时日志，避免被误判为“分析超时”。
- [修复] 问股 Agent 运行增加 LLM 与工具调用的默认单步超时护栏，避免上游无响应导致会话长期卡在“正在制定分析路径...”。
- [新功能] 问股聊天框支持一键清空当前会话并删除聊天日志。
- [改进] 问股上游 LLM 全部失败时补充模型尝试列表与关键网络配置标记，便于排查超时与路由配置问题。
- [改进] 问股本地 OpenAI 兼容服务在工具调用超时时自动降级为无工具调用的文本补偿请求，降低长时间卡住概率。
- [改进] 新增可配置的问股单步 LLM/工具超时（AGENT_LLM_TIMEOUT_S / AGENT_TOOL_TIMEOUT_S），便于本地模型慢速推理场景调参。
